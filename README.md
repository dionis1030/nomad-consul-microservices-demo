...
## Cleaning Up
If you run this demo in Terraform Enterprise, then you should add the following environment variables to your workspace: CONFIRM_DESTROY=1 and TF_WARN_OUTPUT_ERRORS=1.  The first is always needed to destroy infrastructure in TFE. The second is needed in this case to suppress some errors that occur because TFE actually destroys infrastructure by running `terraform plan -destroy -out=<destroy_plan>` followed by `terraform apply <destroy_plan>` which currently complains about the outputs from the module. This happens with Terraform 0.11.6 and lower, but will be fixed in Terraform 0.11.7. Adding the second environment variable suppresses the errors. Neither of these environment variables is needed if using Terraform OSS in which you can simply run `terraform destroy`.  In TFE, you should go to the Settings tab of your workspace and then click the "Queue destroy Plan" button.  Then confirm that you want to destroy when the run reaches the Apply stage of the run.
